...
export default function UserChatRoom({roomData, paramsData, loading}: UserChatRoomProps ) {
    ...
    const { userId, username, profile } = useUserContext()
    ...
    const viewMessages = () => {
        ...
        return (
            roomData[0].messages?.map((message: Messages, index: number) => (
                <View key={index}>
                    <View style={message.sender_id === userId ? styles.sentByYouContainer : { alignSelf: 'flex-start' }}>
                        ...
                        <View 
                            style={
                                [styles.sentByBoth, message.sender_id === userId ? 
                                    {backgroundColor: 'white'} : {borderWidth: 1, borderColor: 'white'}
                                ]
                            }
                        >
                            <Text style={message.sender_id === userId ? {color: 'black'} : {color: 'white'} }>{message.content}</Text>
                        </View>
                    </View>
                </View>
